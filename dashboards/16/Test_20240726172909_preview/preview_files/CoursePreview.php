...
class CoursePreview {
    ...
    public function course_student_passed_percentage_first_attempt($user, $plot): string
    {
        ...
        if ($plot === 'table') {
            if ($user === 'user') {
                $tempText .= '<div id="div_class">'.PHP_EOL.
                    "\t".'<h3 class="main_title">نسبت فراگیرانی که در اولین تلاش، در آزمون قبول شدند به تمام فراگیرانی که در آزمون شرکت کردند (درصد اتمام آزمون) در دوره های sample_teacher</h3>'.PHP_EOL.
                    "\t".'<table class="data-table">'.PHP_EOL."\t\t".'<tr>'.PHP_EOL.
                ...
                $tempText .= '<tr>'.'<td>پایگاه داده مقدماتی</td>'.'<td>آزمون 1</td>'.'<td>20</td>'.'<td>16</td>'.'<td>80%</td>'.'</tr>';

                $tempText .= '<tr>'.'<td>طراحی الگوریتم</td>'.'<td>آزمون 1</td>'.'<td>25</td>'.'<td>18</td>'.'<td>72%</td>'.'</tr>';
            }
            else {
                $tempText .= '<div id="div_class">'.PHP_EOL.
                ...
                $tempText .= '<tr>'.'<td>پایگاه داده پیشرفته</td>'.'<td>آزمون 1</td>'.'<td>20</td>'.'<td>15</td>'.'<td>48%</td>'.'</tr>';

                $tempText .= '<tr>'.'<td>مدار منطقی</td>'.'<td>آزمون مدار منطقی</td>'.'<td>25</td>'.'<td>12</td>'.'<td>72%</td>'.'</tr>';
            ...
            if ($user === 'user') {
                ...
                if ($plot === 'bar') {
                    $chart = (new ChartJS_Bar(['آزمون 1', 'آزمون 2', 'آزمون 3'], [80, 55, 75], 'پایگاه داده مقدماتی', 'درصد اتمام آزمون در دوره پایگاه داده مقدماتی'))->renderChart();
                    $tempText .= $chart;


                    $chart = (new ChartJS_Bar(['آزمون اول پایگاه داده', 'آزمون دوم پایگاه داده'], [60, 70], 'پایگاه داده پیشرفته', 'درصد اتمام آزمون در دوره پایگاه داده پیشرفته'))->renderChart();
                    $tempText .= $chart;
                    ...
                    $chart = (new ChartJS_Pie(['آزمون 1', 'آزمون 2', 'آزمون 3'], [80, 55, 75], 'پایگاه داده مقدماتی', 'درصد اتمام آزمون در دوره پایگاه داده مقدماتی', $isDoughnut))->renderChart();
                    $tempText .= $chart;


                    $chart = (new ChartJS_Pie(['آزمون اول پایگاه داده', 'آزمون دوم پایگاه داده'], [60, 70], 'پایگاه داده پیشرفته', 'درصد اتمام آزمون در دوره پایگاه داده پیشرفته', $isDoughnut))->renderChart();
                    $tempText .= $chart;
                }
                else if (strpos($plot, 'line') >= 0) {
                    $isSmooth = $plot === 'line_smooth';

                    $chart = (new ChartJS_Line(['آزمون 1', 'آزمون 2', 'آزمون 3'], [80, 55, 75], 'پایگاه داده مقدماتی', 'درصد اتمام آزمون در دوره پایگاه داده مقدماتی', $isSmooth))->renderChart();
                    $tempText .= $chart;


                    $chart = (new ChartJS_Line(['آزمون اول پایگاه داده', 'آزمون دوم پایگاه داده'], [60, 70], 'پایگاه داده پیشرفته', 'درصد اتمام آزمون در دوره پایگاه داده پیشرفته', $isSmooth))->renderChart();
                    $tempText .= $chart;
                }
                $tempText .= '<h3 style="font-weight: normal">درصد اتمام آزمون برای دوره طراحی الگوریتم وجود ندارد.</h3><br>';
            }
            else {
                $tempText .= '<div id="div_class">'.PHP_EOL.
                    "\t".'<h3 class="main_title">نسبت فراگیرانی که در اولین تلاش، در آزمون قبول شدند به تمام فراگیرانی که در آزمون شرکت کردند (درصد اتمام آزمون) در تمام دوره ها</h3>'.PHP_EOL;
                ...
                if ($plot === 'bar') {
                    ...
                    $chart = (new ChartJS_Bar(['آزمون اول مدار منطقی', 'آزمون دوم مدار منطقی', 'آزمون سوم مدار منطقی', 'آزمون چهارم مدار منطقی'], [52, 73, 65, 80], 'مدار منطقی', 'درصد اتمام آزمون در دوره مدار منطقی'))->renderChart();
                    $tempText .= $chart;
                    ...
                    $chart = (new ChartJS_Pie(['آزمون اول مدار منطقی', 'آزمون دوم مدار منطقی', 'آزمون سوم مدار منطقی', 'آزمون چهارم مدار منطقی'], [52, 73, 65, 80], 'مدار منطقی', 'درصد اتمام آزمون در دوره مدار منطقی', $isDoughnut))->renderChart();
                    $tempText .= $chart;
                    ...
                    $chart = (new ChartJS_Line(['آزمون اول مدار منطقی', 'آزمون دوم مدار منطقی', 'آزمون سوم مدار منطقی', 'آزمون چهارم مدار منطقی'], [52, 73, 65, 80], 'مدار منطقی', 'درصد اتمام آزمون در دوره مدار منطقی', $isSmooth))->renderChart();
                    $tempText .= $chart;
